...
use super::types::{CrateAttrsFinding, CrateAttrsRule, CrateAttrsRuleId};

use tracing::instrument;

/// Converts crate-attribute markers into open findings.
#[derive(Debug, Default, Clone, Copy)]
pub struct CrateAttrsAssessor;

impl CrateAttrsAssessor {
    pub const ID: &'static str = "crate-attrs-assessor";
}

impl Assessor for CrateAttrsAssessor {
    ...
    fn assess(&self, view: AssessView<'_>) -> CordialResult<Vec<Box<dyn Finding>>> {
        ...
        for marker in markers {
            ...
            let Some(rule_id) = CrateAttrsRuleId::from_attr(rule_value) else {
                continue;
            };
            let context = node
                .attr("context")
