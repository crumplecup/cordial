...
//!
//! **What.** Checks whether visible modules are justified public paths.
//!
//! **Why.** `pub mod` is a promise of a public path. A thin module or a
//! public child of a private parent splits crate-internal navigation without
//! buying a real API.
//!
//! **Flags.** Small crates that should stay flat, visible modules with too
//! few leaf names, and child modules whose visibility exceeds their parent.
//!
//! **Ignores.** Pub fields belong to `derives`. Module mass belongs to
//! `modularity`. Conditional-code placement belongs to `cfg_scatter`.
//!
//! **Outputs.** `{store}/findings/visibility.checklist.md`,
//! `visibility-summary.md`, and CSV.
//!
//! **Config.** `[visibility]` owns thresholds, `prefer_root`, `mod_thin_skip`,
//! and `enabled`. Register [`VISIBILITY_ETIQUETTE`]. Policy:
//! `docs/planning/visibility-etiquette.md`.
