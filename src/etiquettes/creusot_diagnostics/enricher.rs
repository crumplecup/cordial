...
use super::scan::scan_crate_creusot_diagnostics;

use tracing::instrument;

/// Materializes Creusot diagnostic expression nodes in the IR graph.
#[derive(Debug, Default, Clone, Copy)]
pub struct CreusotDiagnosticInventoryEnricher;

impl CreusotDiagnosticInventoryEnricher {
    pub const ID: &'static str = "creusot-diagnostic-inventory";
}

impl IrEnricher for CreusotDiagnosticInventoryEnricher {
    ...
    fn enrich(&self, view: EnrichView<'_>) -> CordialResult<()> {
        ...
        let policy = crate::config::load_session_config(session)
            .creusot_diagnostics()
            .clone();
        let records = scan_crate_creusot_diagnostics(&crate_root, ir.crate_name(), &policy)?;
