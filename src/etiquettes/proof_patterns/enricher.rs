...
use super::scan::scan_crate_proof_patterns;

use tracing::instrument;

/// Materializes proof-pattern expression nodes in the IR graph.
#[derive(Debug, Default, Clone, Copy)]
pub struct ProofPatternInventoryEnricher;

impl ProofPatternInventoryEnricher {
    pub const ID: &'static str = "proof-pattern-inventory";
}

impl IrEnricher for ProofPatternInventoryEnricher {
    ...
    fn enrich(&self, view: EnrichView<'_>) -> CordialResult<()> {
        ...
        let records = scan_crate_proof_patterns(&crate_root)?;
        ...
        for record in records {
            ...
            ir.set_attr(
                node,
                "tracked_params",
                serde_json::Value::String(record.tracked_params.join(", ")),
            )?;
            ir.set_attr(
                node,
                "recommends",
                serde_json::Value::String(record.recommends.join(", ")),
            )?;
