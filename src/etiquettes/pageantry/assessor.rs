...
use super::types::{PageantryFinding, PageantryRule, PageantryRuleId};

use tracing::instrument;

/// Converts pageantry markers into open findings.
#[derive(Debug, Default, Clone, Copy)]
pub struct PageantryAssessor;

impl PageantryAssessor {
    pub const ID: &'static str = "pageantry-assessor";
}

impl Assessor for PageantryAssessor {
    ...
    fn assess(&self, view: AssessView<'_>) -> CordialResult<Vec<Box<dyn Finding>>> {
        ...
        for marker in markers {
            ...
            let Some(rule_id) = PageantryRuleId::from_attr(rule_value) else {
                continue;
            };
            let context = node
                .attr("context")
            ...
            findings.push(Box::new(PageantryFinding {
                rule: PageantryRule::new(rule_id),
