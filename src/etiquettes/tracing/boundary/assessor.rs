...
use super::types::{BOUNDARY_SITE_LABEL, BoundaryFinding, BoundaryRule, BoundaryRuleId};

use tracing::instrument;

/// Converts binary-error-boundary markers into open findings.
#[derive(Debug, Default, Clone, Copy)]
pub struct BoundaryAssessor;

impl BoundaryAssessor {
    pub const ID: &'static str = "tracing-boundary-assessor";
}

impl Assessor for BoundaryAssessor {
    ...
    fn assess(&self, view: AssessView<'_>) -> CordialResult<Vec<Box<dyn Finding>>> {
        ...
        for marker in markers {
            ...
            let Some(rule_id) = BoundaryRuleId::from_attr(rule_value) else {
                continue;
            };
            let context = node
                .attr("context")
            ...
            findings.push(Box::new(BoundaryFinding {
                rule: BoundaryRule::new(rule_id),
