...
use super::types::{SUBSCRIBER_SITE_LABEL, SubscriberFinding, SubscriberRule, SubscriberRuleId};

use tracing::instrument;

/// Converts tracing-subscriber markers into open findings.
#[derive(Debug, Default, Clone, Copy)]
pub struct SubscriberAssessor;

impl SubscriberAssessor {
    pub const ID: &'static str = "tracing-subscriber-assessor";
}

impl Assessor for SubscriberAssessor {
    ...
    fn assess(&self, view: AssessView<'_>) -> CordialResult<Vec<Box<dyn Finding>>> {
        ...
        for marker in markers {
            ...
            let Some(rule_id) = SubscriberRuleId::from_attr(rule_value) else {
                continue;
            };
            let context = node
                .attr("context")
            ...
            findings.push(Box::new(SubscriberFinding {
                rule: SubscriberRule::new(rule_id),
